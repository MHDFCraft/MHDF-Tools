...
import cn.ChengZhiYa.ChengToolsReloaded.Tasks.Money_Task;
...
public class BalTop implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        for (int i=1;i<(main.main.getConfig().getInt("EconomySettings.BalTopInt")+1);i++) {
    ...
    public String[] getTop(int top) {
        try {
            Map.Entry<String, Double> entry = Money_Task.getPlayer(top);
            if (entry == null) {
