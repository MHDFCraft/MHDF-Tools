...
import cn.ChengZhiYa.ChengToolsReloaded.Commands.Tpa.Tpa;
import cn.ChengZhiYa.ChengToolsReloaded.Commands.Tpa.TpaAccept;
import cn.ChengZhiYa.ChengToolsReloaded.Commands.Tpa.TpaDefuse;
import cn.ChengZhiYa.ChengToolsReloaded.Commands.TpaHere.TpaHere;
import cn.ChengZhiYa.ChengToolsReloaded.Commands.TpaHere.TpaHereAccept;
import cn.ChengZhiYa.ChengToolsReloaded.Commands.TpaHere.TpaHereDefuse;
...
public final class main extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        if (getConfig().getBoolean("ScoreboardSettings.Enable")) {
            BukkitTask Scoreboard = new Scoreboard_Task().runTaskTimer(this, 0L, 20L);
        ...
        if (getConfig().getBoolean("TimeMessageSettings.Enable")) {
            BukkitTask TimeMessage = new TimeMessage_Task().runTaskTimer(this, 0L, getConfig().getInt("TimeMessageSettings.Delay") * 20L);
            IntHashMap.Set("TimeMessageTaskId", TimeMessage.getTaskId());
        }
        if (getConfig().getBoolean("VanillaOpWhitelist.Enable")) {
            BukkitTask WhiteListTask = new VanillaOpWhitelist_Task().runTaskTimer(this, 0L, 20L);
            IntHashMap.Set("OpWhiteListTaskID", WhiteListTask.getTaskId());
        }
        if (getConfig().getBoolean("ChatSettings.ChatDelayEnable")) {
            BukkitTask ChatDelayTime = new ChatDelay_Time().runTaskTimer(this, 0L, 20);
        ...
        if (getConfig().getBoolean("LoginSystemSettings.Enable")) {
            ...
            BukkitTask LoginMessage = new LoginMessage_Task().runTaskTimer(this, 0L, 20L);
        ...
        if (getConfig().getBoolean("TpaEnable")) {
            ...
            BukkitTask TpaTime = new Tpa_Time().runTaskTimer(this, 0L, 20);
        ...
        if (getConfig().getBoolean("TpahereEnable")) {
            ...
            BukkitTask TpahereTime = new TpaHere_Time().runTaskTimer(this, 0L, 20);
            IntHashMap.Set("TpaTimeTaskId", TpahereTime.getTaskId());
        }
