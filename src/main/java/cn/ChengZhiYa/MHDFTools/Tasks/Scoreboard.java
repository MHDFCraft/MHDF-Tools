            ...
            if (!Bukkit.getOnlinePlayers().isEmpty()) {
                try {
                    for (Player player : Bukkit.getOnlinePlayers()) {
                        ...
                        ObjectiveHasMap.getHasMap().put(player.getName() + "_Objective", ScoreboardHasMap.getHasMap().get(player.getName() + "_Scoreboard").registerNewObjective(ChatColor(player, Objects.requireNonNull(MHDFTools.instance.getConfig().getString("ScoreboardSettings.Title"))), "dummy"));
                        ...
                        for (int i = 0; i < MHDFTools.instance.getConfig().getStringList("ScoreboardSettings.Lines").size(); i++) {
                            ...
                            if (MHDFTools.instance.getConfig().getStringList("ScoreboardSettings.Lines").get(i) == null || MHDFTools.instance.getConfig().getStringList("ScoreboardSettings.Lines").get(i).isEmpty()) {
                                ...
                                ScoreMessage = ObjectiveHasMap.getHasMap().get(player.getName() + "_Objective").getScore(ChatColor(player, MHDFTools.instance.getConfig().getStringList("ScoreboardSettings.Lines").get(i)));
                            }
