...
public final class SetWarp implements TabExecutor {
    @Override
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                ...
                if (ifWarpExists(warpName)) {
                    setWarp(warpName, new SuperLocation(player.getLocation()));
                } else {
                    addWarp(warpName, new SuperLocation(player.getLocation()));
                }
                player.sendMessage(i18n("Warp.SetDone"));
            } else {
